...
Additionally, splitting the dataset randomly can introduce sequence leakage, as laps from the same race may appear in both training and test sets. A grouped split by race was tested, resulting in lower performance, highlighting the model’s reliance on race-specific patterns.
...
Use time-series or sequential models (e.g. LSTM)
